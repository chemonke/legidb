...
1) Option A (simplest): `nix run .#db-start` (nix provides `mysqld`/`mysql` and keeps everything under `/tmp/legidb-mariadb`).
...
```
python run.py
# or: nix run .#app   (uses the flake-provided Python env)
```
6) Stop and clean:
`nix run .#db-stop` or `scripts/stop_ephemeral_mariadb.sh [--clean]` (adds `--clean` to delete `/tmp/legidb-mariadb`).
